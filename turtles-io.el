...
(defun turtles--print-msg (msg)
  "Print the object representation of MSG to the current buffer."
  ...
  (let ((print-unreadable-function
         (lambda (obj _escape)
           (format "(unreadable %s)" (type-of obj))))
        (print-length nil)
        (print-level nil)
        (print-circle t)
        (print-quoted t)
  ...
  (with-current-buffer (process-buffer (turtles-io-conn-proc conn))
    ...
    (while (save-excursion
      ...
      (let ((end (match-end 0)) obj)
        ;; (let ((set-message-function nil))
        ;;   (message "read: %s" (buffer-substring-no-properties (point-min) end)))
